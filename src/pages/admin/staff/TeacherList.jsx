    ...
    <div className="p-6 bg-white rounded shadow">
      ...
      {permStaff && (
        <PermissionsEditor
          ...
          onSuccess={() => { setPermStaff(null); loadTeachers(); }}
        />
      )}
      ...
      {teachers.length === 0 ? (
        ...
        <table className="w-full border border-[#0a1a44] rounded overflow-hidden">
          ...
          <tbody>
            {teachers.map((t, idx) => (
              <tr key={t._id} className={idx % 2 === 0 ? "bg-gray-50" : "bg-white"}>
                <td className="p-3">{t.fullName}</td>
                <td className="p-3">{t.email}</td>
                <td className="p-3">{t.phone || "-"}</td>
                <td className="p-3">
                  <span className={`px-2 py-1 rounded text-sm ${t.isActive ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
